...
# Instance mask generation (SAM3 — for Trace3D integration)
python scripts/generate_masks.py -s <dataset_path>                              # text-prompt "object"
...
- **`scripts/generate_masks.py`** — SAM3-based mask generation. Text-prompt or grid-point mode. Saves `<source>/masks/<image_name>.png`.
- **`utils/instance_utils.py`** — Core algorithms: `build_instance_weight_matrix` (project Gaussians → read mask label), `majority_vote_merge` (Jaccard + Union-Find), `assign_labels`, loss functions (`instance_seg_loss_ce`, `depth_edge_loss`).
- Instance fields on `GaussianModel`: `_instance_id` (long), `_instance_entropy` (float) — non-optimizable, synced during densify/prune.
- Key args: `instance_seg_from_iter=7000`, `instance_update_interval=500`, `lambda_seg=0.1`, `lambda_depth_edge=0.05`, `tau_affinity=0.3`.
...
- `sam3` — SAM 3 (Segment Anything Model 3) for instance mask generation
