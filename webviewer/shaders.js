...
// Segment uniforms
uniform sampler2D uSegmentTex;
uniform int uSegMode;  // 0=off, 1=category color, 2=instance color
uniform bool uCatVisible0;
uniform bool uCatVisible1;
uniform bool uCatVisible2;
uniform bool uCatVisible3;
uniform vec3 uCatColor0;
uniform vec3 uCatColor1;
uniform vec3 uCatColor2;
uniform vec3 uCatColor3;
...
void main() {
    ...
    // Segment color mode processing
    if (uSegMode > 0) {
        vec4 segData = texelFetch(uSegmentTex, texCoord(idx), 0);
        int semCat = int(segData.x + 0.5);
        float instId = segData.y;

        // Visibility check per category
        bool visible = true;
        if (semCat == 0) visible = uCatVisible0;
        else if (semCat == 1) visible = uCatVisible1;
        else if (semCat == 2) visible = uCatVisible2;
        else if (semCat == 3) visible = uCatVisible3;
        ...
        if (uSegMode == 1) {
            // Category color mode
            if (semCat == 0) color = uCatColor0;
            else if (semCat == 1) color = uCatColor1;
            else if (semCat == 2) color = uCatColor2;
            else if (semCat == 3) color = uCatColor3;
        } else if (uSegMode == 2) {
            // Instance color mode (golden angle hash)
            float hue = mod(instId * 137.508, 360.0);
            color = hsvToRgb(hue, 0.85, 0.95);
        }
    }
