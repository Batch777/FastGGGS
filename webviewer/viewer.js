...
let sgDegree = 0;
let bgColor = [0, 0, 0];
let kernelSize = 0.0; // matches CUDA kernel_size (0.0 for GGGS scenes)
...
function uploadGaussianData(splatData) {
    ...
    showStatus(`Loaded ${gaussianCount.toLocaleString()} Gaussians (SH degree ${maxSHDegree}${sgInfo})`);
    controls.dirty = true;
}
...
function renderLoop(time) {
    ...
    gl.uniform1f(uniforms.uKernelSize, kernelSize);
...
async function loadFile(file) {
    ...
    if (file.name.endsWith('.splat')) {
        splatData = parseSplat(buffer);
    } else {
        splatData = parsePLY(buffer, (progress) => {
            showStatus(`Parsing PLY... ${Math.round(progress * 100)}%`);
        });
    }
...
function updateIterSelect() {
    ...
    for (const iter of selectedModel.iterations) {
        ...
        opt.textContent = `${iter.iteration}${plyInfo || splatInfo}`;
...
async function loadServerModel() {
    ...
    // Prefer .ply (has SH/SG/filter_3D), fall back to .splat
    const fileInfo = iter.files.ply || iter.files.splat;
    if (!fileInfo) return;
    ...
    try {
        ...
        // Set kernel_size from model config (matches CUDA rasterizer)
        if (selectedModel.config && selectedModel.config.kernel_size !== undefined) {
            kernelSize = selectedModel.config.kernel_size;
        }
