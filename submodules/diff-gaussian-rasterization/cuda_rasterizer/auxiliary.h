...
// }
__forceinline__ __device__ float3 dnormvdv(float3 v, float3 dv) {
    float inv_norm = rnorm3df(v.x, v.y, v.z);
    float3 vn      = {v.x * inv_norm, v.y * inv_norm, v.z * inv_norm};

    float3 dnormvdv;
    dnormvdv.x = ((+1.f - vn.x * vn.x) * dv.x - vn.y * vn.x * dv.y - vn.z * vn.x * dv.z) * inv_norm;
    dnormvdv.y = (-vn.x * vn.y * dv.x + (1.f - vn.y * vn.y) * dv.y - vn.z * vn.y * dv.z) * inv_norm;
    dnormvdv.z = (-vn.x * vn.z * dv.x - vn.y * vn.z * dv.y + (1.f - vn.z * vn.z) * dv.z) * inv_norm;
    return dnormvdv;
}
...
__forceinline__ __device__ bool in_frustum(const float3 p_orig,
...
__forceinline__ __device__ static T pythag(T const& a, T const& b) {
    ...
    if (glm_modification::equal<T>(absb, 0, epsilon))
...
// SNUGBOX (Compact Bounding Box) functions from Speedy-Splat/FastGS.
// Computes tight ellipse-based tile bounds instead of conservative AABB.
...
#define CUDA_VERSION_AT_LEAST_12_9 (__CUDACC_VER_MAJOR__ > 12 || (__CUDACC_VER_MAJOR__ == 12 && __CUDACC_VER_MINOR__ >= 9))
#else
#define CUDA_VERSION_AT_LEAST_12_9 0
#endif
